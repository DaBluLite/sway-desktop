...
interface AlbumCarouselProps {
  ...
  onAlbumPlay: (album: ExtendedAlbum) => void
}
...
export const AlbumCarousel: React.FC<AlbumCarouselProps> = ({
  ...
  const loadMoreAlbums = useCallback(async () => {
    ...
    try {
      const newAlbums = await fetchAlbums(offset, 15)
  ...
  }, [updateArrowVisibility, albums.length])
  ...
  if (!hasInitializedRef.current && albums.length === 0) return null
        ...
        <div className="station-carousel-scroll" ref={scrollContainerRef}>
          <div className="station-carousel-content">
            {albums.map((album, index) => (
              <AlbumCard key={`${album.id}-${index}`} album={album} onPlay={onAlbumPlay} />
            ))}
            {hasMore && (
