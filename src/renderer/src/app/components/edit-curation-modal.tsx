...
import { EditCurationModalProps } from '../types/modal'
...
export const EditCurationModal: React.FC<EditCurationModalProps> = ({
  onClose,
  collectionId
}: EditCurationModalProps) => {
  const { collections, updateCollection } = useCurations()
  const collection = collections.find((c) => c.id === collectionId)

  const [name, setName] = useState(collection?.name || '')
  const [description, setDescription] = useState(collection?.description || '')
  const [icon, setIcon] = useState(collection?.icon || EMOJI_PRESETS[0])
  const [color, setColor] = useState(collection?.color || COLOR_PRESETS[0].class)
      ...
      <div className="modal max-w-md!" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <div className="flex items-center gap-2">
            <h2 className="text-xl font-semibold text-white">Edit Curation</h2>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="flex flex-wrap gap-2">
              {COLOR_PRESETS.map((c) => (
                <button
                  ...
                  className={`size-10 rounded-md transition border bg-linear-to-br ${c.class} ${
